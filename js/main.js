...
var dragonSmall = new dragon(
  ...
  0,
  1
) /*创建进化龙，small小龙，最后三个：HP为10，经验为0，龙为 1-小龙*/
...
function startGame() {
  if (isStart == 0) {
    ...
    s1 = setInterval(createEnemy, newEnemyCreate) /*创建敌人，enemy*/
    s2 = setInterval(enemyMove, newEnemyMove) /*敌人移动，每250ms移动一次*/
    ...
    scoreBox.style.display = 'block' /*显示记分 板*/
...
overY.onmouseup = function () {
  ...
  /* window.location.href="MapleStory.html";666*/
}
...
document.body.onkeyup = function () {
  ...
  if (e.keyCode == 13 && dragonSmall.isDead == false) {
    /*回车 大招*/
    limitStrokes = 0
  }
}
...
function skill() {
  ...
  this.skllBgNode.style.opacity = 0.8
...
function dragon(imgSrc, x, y, speed, hp, exp, who) {
  ...
  this.attackDragon = function () {
    if (dragonIsAtt == 0 && dragonSmall.isDead == false) {
      /*攻击特效*/
...
/*龙的魔法弹*/
function attBall(imgSrc, x, y, speed, power) {
  /*魔法弹对象*/
...
/*BOSS魔法弹*/
function attBallBoss(imgSrc, x, y, speed, power) {
  /*魔法弹对象*/
...
function enemyBird(imgSrc, x, y, speedX, speedY, hp, exp) {
  ...
  this.move = function () {
    ...
    if (this.exp == 40) {
      ...
      if (k >= 0.5) {
        /*向下移动+*/
        if (parseInt(this.enemyNode.style.top) + p > mainHeight - 200)
          k = 0.2 /*如果大于下边距 执行向上移动*/
        else this.enemyNode.style.top = parseInt(this.enemyNode.style.top) + p + 'px'
      }
      if (k < 0.5) {
        /*向上移动-*/
        if (parseInt(this.enemyNode.style.top) - p < 90) {
          /*如果小于上边距 执行向上移动*/
          this.enemyNode.style.top = parseInt(this.enemyNode.style.top) + p + 'px'
        }
        this.enemyNode.style.top = parseInt(this.enemyNode.style.top) - p + 'px'
      }
      if (parseInt(this.enemyNode.style.right) < 40)
        /*如果还没有完全显示出来就移动，显示出后停止x移动*/
        this.enemyNode.style.right = parseInt(this.enemyNode.style.right) + speedX + 'px'
    } /*boss*/ else if (this.exp >= 20) {
      /*ghost*/
      if (k >= 0.5) {
        /*向下移动+*/
        if (parseInt(this.enemyNode.style.top) + p > mainHeight - 110)
          k = 0.2 /*如果大于下边距 执行向上移动*/
        else this.enemyNode.style.top = parseInt(this.enemyNode.style.top) + p + 'px'
      }
      if (k < 0.5) {
        /*向上移动-*/
        if (parseInt(this.enemyNode.style.top) - p < 80) {
          /*如果小于上边距 执行向上移动*/
          this.enemyNode.style.top = parseInt(this.enemyNode.style.top) + p + 'px'
        } else this.enemyNode.style.top = parseInt(this.enemyNode.style.top) - p + 'px'
      }
  ...
  this.attack = function () {
    ...
    if (randomAttack < 0.7 && issetBOSS == 1 && isStart == 1) {
      /*小于0.7攻击，变成攻击状态*/
  ...
  function attackEnd() {
    /*攻击结束，变回移动状态*/
    if (issetBOSS) {
      var bossAttackBall = new attBallBoss(
        'images/enemy/boss/attackBall.gif',
        parseInt(bossEnemy.style.right) + 250,
        parseInt(bossEnemy.style.top) + 50,
        10,
        7
      )
...
function createEnemy() {
  ...
  if (random > 90 && issetBOSS == 0) {
    /*随机产生BOSS，并且BOSS一次只出现一个*/
    if (limitBoss == 0 && issetBOSS == 0) {
      issetBOSS = 1 /*记录BOSS*/
      var enemy = new enemyBird(
        'images/enemy/boss/',
        -200,
        parseInt(Math.random() * (mainHeight - 290) + 90),
      ...
      var enemy = new enemyBird(
        'images/enemy/bird/',
        -200,
        parseInt(Math.random() * (mainHeight - 110) + 50),
        parseInt(Math.random() * 4 + 1),
        0,
        1,
        1
    ...
    var enemy = new enemyBird(
      'images/enemy/ghost/',
      -200,
      parseInt(Math.random() * (mainHeight - 190) + 80),
    ...
    var enemy = new enemyBird(
      'images/enemy/plane/',
      -200,
      parseInt(Math.random() * (mainHeight - 150) + 50),
      parseInt(Math.random() * 7 + 1),
      0,
      5,
      7
    )
...
function dragonisDead() {
  if (!dragonSmall.isDead) {
    if (dragonSmall.hp <= 0) {
      ...
      howHp.style.width =
        parseInt((dragonSmall.hp / dragonSmall.maxHp) * 100) + '%' /*百分比显示血量*/
    }
  }
}
/*龙死亡动作*/
function dragonDie() {
  ...
  dragonSmall.enemyNode.style.left =
    parseInt(dragonSmall.enemyNode.style.left) - parseInt(imgWidth / 4) + 'px'
...
/*龙被hit 闪烁 无敌*/
function flashing() {
  if (dragonSmall.enemyNode.className == 'style') dragonSmall.enemyNode.className = 'hit'
  else dragonSmall.enemyNode.className = 'style'
  dragonSmall.status++
...
function enemyMove() {
  for (var i in enemyList) {
    if (!enemyList[i].isDead) {
      if (enemyList[i].status == 0) {
      ...
      } else if (enemyList[i].status == 1 && enemyList[i].hp > 0) {
        /*hit效果结束 未死亡王*/
        ...
        if (enemyList[i].exp == 1) {
          /*7 23 40*/
          birdDie.play()
        } else if (enemyList[i].exp == 7) {
          /*7 23 40*/
          planeDie.play()
        } else if (enemyList[i].exp == 23) {
          /*7 23 40*/
          ghostDie.play()
        } else if (enemyList[i].exp == 40) {
          /*7 23 40*/
          bossDie.play()
        }
        setTimeout(
          newThing(
            parseInt(enemyList[i].enemyNode.style.right),
            parseInt(enemyList[i].enemyNode.style.top)
          ),
          500
        )
        enemyList[i].isDead = true
        ...
        howExp.style.height =
          parseInt((dragonSmall.exp / expList[dragonSmall.who]) * 0.6 * 100) + 40 + '%'
...
function fl() {
  ...
  else dragonSmall.enemyNode.style.visibility = 'visible'
  ii++
  if (ii >= 10) {
    LevelUp.play()
...
function dragonChange() {
  ...
  dragonSmall = new dragon(
    dragonSrc,
    parseInt(dragonSmall.enemyNode.style.left),
    parseInt(dragonSmall.enemyNode.style.top),
    2,
    parseInt(dragonSmall.who * 10),
    0,
    dragonSmall.who
  ) /*创建进化龙，small小龙，最后三个：HP为10，经验为0，龙的等级*/
  /*进化完全*/
  changing = 0
}
/*龙魔法弹移动*/
    ...
    for (var j in enemyList) {
      ...
      if (
        ...
        if (ballList[i].power == 15) {
          ...
          ballList[i].ballNode.style.left =
            mainWidth - parseInt(enemyList[j].enemyNode.style.right) - eW - 50 + 'px'
          ...
          ballList[i].ballNode.style.left =
            mainWidth - parseInt(enemyList[j].enemyNode.style.right) - eW + 'px'
        } else if (ballList[i].power == 2) {
          dragon2.play()
          ballList[i].ballNode.style.top = parseInt(enemyList[j].enemyNode.style.top) - 10 + 'px'
          ballList[i].ballNode.style.left =
            mainWidth - parseInt(enemyList[j].enemyNode.style.right) - eW - 10 + 'px'
...
function hitCheckBoss() {
  ...
  for (var i in bossList) {
    ...
    if (
      ...
      /*魔法弹的变化*/
      bossList[i].status = 8
      bossList[i].ballBossNode.style.right =
        parseInt(bossList[i].ballBossNode.style.right) + eW + 'px'
      bossList[i].ballBossNode.src = bossList[i].hitSrc /*更换魔法弹为hit效果*/
      /*敌人的变化*/
      //main.removeChild(ballList[i].ballNode);/*删除魔法弹*/
      //ballList.splice(i,1);/*删除魔法弹*/

      if (!dragonSmall.isInvincible) {
        /*如果不无敌*/
...
// ++++++++++++++++++++++++++++++++++++=
// 参数设置
var settingContainer = document.getElementsByClassName('settingContainer')
...
function settingContainerHide() {
  for (i = 0; i < 5; i++) {
    settingContainer[i].style.display = 'none'
  }
}
mapSelect.onclick = function () {
  settingContainerHide()
  mapContainer.style.display = 'block'
}
skinSelect.onclick = function () {
  // 显示皮肤框
  settingContainerHide()
  skinContainer.style.display = 'block'
}
modeSelect.onclick = function () {
  // 显示模式框
  settingContainerHide()
...
document.getElementsByClassName('item')[1].onclick = function () {
  mapImgNo = 1
}
document.getElementsByClassName('item')[2].onclick = function () {
  mapImgNo = 2
}
document.getElementsByClassName('item')[3].onclick = function () {
  mapImgNo = 3
}
document.getElementsByClassName('item')[4].onclick = function () {
  mapImgNo = 4
}
document.getElementsByClassName('item')[5].onclick = function () {
  mapImgNo = 5
}
function mapSelecting() {
  if (mapImgNo == -1) {
    main.style.background = "url('images/bg.jpg') center top"
  } else {
    var mapSrc = 'images/background/bg' + mapImgNo + '.jpg'
    main.style.background = 'url(' + mapSrc + ') center top'
  }
  main.style.backgroundSize = 'cover'
}

// 皮肤
newDragonImg.onclick = function () {
  dragonSrc = 'images/newDragon/'
}
// 游戏模式
var modes = document.getElementsByClassName('list-group-item')
modes[0].onclick = function () {}
modes[1].onclick = function () {
  newHP = 999999999
}
modes[2].onclick = function () {
  newCountDown = 901
}
modes[3].onclick = function () {
  newEnemyCreate = 100
  newEnemyMove = 10
  newHP = 100
}
