...
function startGame(){
    if(isStart==0){
        ...
        dragonSmall = new dragon(dragonSrc,10,300,2,newHP,0,newWho);/*创建进化龙，small小龙，最后三个：HP为10，经验为0，龙为 1-小龙*/
        /*加载游戏*/
    ...
    for(var i in enemyList){
        if(!enemyList[i].isDead){
            if(enemyList[i].status==0){
                enemyList[i].move();
                if(parseInt(enemyList[i].enemyNode.style.right)>1536){/*消失*/
...
function dragonChange(){/*龙进化*/
    ...
    clearInterval(intAttackDragonBall);
    /*新的龙*/
    dragonSmall = new dragon(dragonSrc,parseInt(dragonSmall.enemyNode.style.left),parseInt(dragonSmall.enemyNode.style.top),2,parseInt(dragonSmall.who*10),0,dragonSmall.who);/*创建进化龙，small小龙，最后三个：HP为10，经验为0，龙的等级*/
    /*进化完全*/
    changing=0;
}
/*龙魔法弹移动*/
    ...
    for(var i in ballList){
        if(!ballList[i].isDead){
            if(ballList[i].status==0){
                ballList[i].move();
                if(parseInt(ballList[i].ballNode.style.left)>1536){/*离开视野后 魔法弹消失*/
    ...
    for(var i in bossList){
        if(!bossList[i].isDead){
            if(bossList[i].status==0){
                bossList[i].move();
                if(parseInt(bossList[i].ballBossNode.style.right)>1536){/*BOSS魔法弹消失*/
